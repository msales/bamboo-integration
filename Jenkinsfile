...
pipeline {
  ...
  environment {
    ...
    JIRA_PROJECT = "BLT"
    GIT_TAG = sh script: 'git describe --tags', returnStdout: true
    ...
    // def git_log = sh(returnStdout: true, script: 'git log $git_tag_old..HEAD --oneline | grep -Eo \'([A-Z0-9]{3,}-)([0-9]+)\' | uniq').trim()
  }
